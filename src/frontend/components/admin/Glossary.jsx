...
// modules imports
import AddGlossary from './utils/AddGlossary.jsx';
import Loading from '../Loading.jsx';
import ViewGlossary from './utils/ViewGlossary.jsx';
    ...
    <Toolbar className={clsx(classes.root)}>
      <Grid container spacing={2} alignItems="center" justify="flex-start">
        ...
        <Grid item>
          ...
          <AddGlossary open={open} onClose={handleClose} library={library} />
        </Grid>
      </Grid>
    </Toolbar>
  );
...
export default function EnhancedTable(props) {
  ...
  const handleClose = note => {
    if (note) {
      let editedGlossaries = [...rows];
      editedGlossaries[index] = note;
      setRows(editedGlossaries);
    }
    setOpen(false);
  };
  ...
  React.useEffect(() => {
    ...
    fetch('/api/v1/glossaries')
      ...
      .then(glossaries => {
        if (status === 200) {
          setRows(glossaries.data);
          setIsLoaded(true);
          return;
        } else {
          processError(glossaries);
